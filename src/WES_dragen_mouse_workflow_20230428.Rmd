---
title: "WES dragen workflow with mouse samples"
author: "Bingfang Xu"
date: "4/28/2023"
...
Download from https://hpcdmeweb.nci.nih.gov/collection?action=view&path=/FNL_SF_Archive/PI_Lab_Daniel_Soppet/Project_DanielSoppet_CS034244_12exome_041723&source=browse&init
...
# Dargen somatic calling

### VCF to VEP on Biowulf (optional)
```
...
```
### check vcf FORMAT, make changes in _2Vep_extractInfo_somaticList.py and _4single_clone_output.py if needed.
### check sample names, make changes of clone name if needed.
### input files:*_vs_1_RPZ27911.hard-filtered.vep.vcf
### Filter by AF and DP.
...
Use  joint.hard-filtered.vcf.gz or joint.hard-filtered.vep.vcf.gz as input.
...
java -Xmx8g -jar $GATK_JAR -T SelectVariants -R /data/xubr/mm10_ordered_all.fa -V /data/xubr/joint.hard-filtered.vep.vcf.gz --discordance /data/xubr/sorted2_mgp.v4.all.dbSNP_Sample_129S5SvEvBrd.vcf -o joint_noncommonSNP.vep.vcf -U ALLOW_SEQ_DICT_INCOMPATIBILITY

### Remove variants already existed in RPZ113 using data from three PRZ113 sequencing(200ngRPZ113, 3ngRPZ113, and RPZ0113)
java -Xmx8g -jar $GATK_JAR -T SelectVariants -R /data/xubr/mm10_ordered_all.fa -V /data/xubr/joint_noncommonSNP.vep.vcf --discordance /data/xubr/combined_GATK_3RPZ113.vcf -o /data/xubr/joint_noncommonSNP_nonRPZ113.vep.vcf -U ALLOW_SEQ_DICT_INCOMPATIBILITY

### Remove non-exon variants
java -Xmx8g -jar $GATK_JAR -T SelectVariants -R mm10_ordered_all.fa -L S0276129_Regions_mm10.bed --variant joint_noncommonSNP_nonRPZ113.vep.vcf -o Ontarget_joint_noncommonSNP_nonRPZ113.vep.vcf
